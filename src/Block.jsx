...
function Block({
  ...
  const [friendStakes,       setFriendStakes]       = useState([]);
  ...
  // ── HASHING ──
  // ── UPDATED HASHING EFFECT ──
useEffect(() => {
  if (!isMining && !isSelecting && !showMiningRace) {
    const txData  = JSON.stringify({ sender, receiver, amount });
    // Keep the posMode logic as you had it
    const posMode = hasWonLottery || (blockMode === 'POS' && !hash.startsWith('00'));
    
    // previousHash MUST be a dependency to trigger the domino effect
    const text    = index + previousHash + txData + nonce + (posMode ? validator + stake : '');
    const newHash = sha256(text).toString();

    if (newHash !== hash) {
      setHash(newHash);
      // This sends the new (incorrect) hash to the NEXT block's previousHash prop
      onUpdate(index, { 
        hash: newHash, nonce, sender, receiver, amount, 
        signature, txSignature, signerPubKey, 
        originalTxHash: savedOriginalTxHash, validator, 
        stake, createdWith: savedCreatedWith, friendSigned 
      });
    }
  }
}, [sender, receiver, amount, previousHash, index, nonce, signature]);

  // ── TAMPER DETECTION FOR FRIEND SIGNED BLOCKS ──
  useEffect(() => {
    if (!isMounted.current) return;
    if (previousHash !== prevPreviousHashRef.current) {
      if (signature||hash.startsWith('00')) tamperedRef.current = true;
    }
   prevPreviousHashRef.current = previousHash;
}, [previousHash, signature, hash]);

 // A PoW block is valid if it starts with 00 AND hasn't been tampered by a previous block
const isPowValid = hash.startsWith('00') && !tamperedRef.current;

// A PoS block is valid if it has a signature AND hasn't been tampered by a previous block
const isPosValid = signature !== '' && !tamperedRef.current;
...
const isValid = (index === 1 && hash === GENESIS_HASH) || isPowValid || isPosValid;
  ...
  useEffect(() => {
    ...
    const blockCreatedWithPOS = savedCreatedWith === 'POS' || blockMode === 'POS';
    const hasPosSignature = !!signature;
    const consensusMismatch = modeChanged || (blockCreatedWithPOS && consensus === 'POW') || (!blockCreatedWithPOS && consensus === 'POS') || (hasPosSignature && consensus === 'POW');
    if (isMounted.current && !consensusMismatch && wasValidRef.current && !isValid && !isMining && !isSelecting) {
  ...
  }, [isValid, consensus, isMining, isSelecting, index, addLog, friendSigned, validator, signature]);
  ...
  const runValidatorLottery = () => {
    ...
    setTimeout(() => {
      ...
      const youWinRoll = Math.random() < 0.30;
      const yourNumber = stake;

      const friendNumbers = FRIEND_LIST.map(f => ({
        ...f,
        stake: youWinRoll
          ? Math.floor(Math.random() * Math.max(yourNumber - 1, 1))
          : yourNumber + Math.floor(Math.random() * 200) + 1,
      }));
      setFriendStakes(friendNumbers);

      const allEntries = [
        { name: 'YOU', emoji: '🧑‍💻', stake: yourNumber, isYou: true },
        ...friendNumbers.map(f => ({ ...f, isYou: false })),
      ];

      const winner = allEntries.reduce((best, curr) => curr.stake > best.stake ? curr : best);

      if (addLog) addLog(
        `🎲 NUMBERS — YOU: ${yourNumber} | ${friendNumbers.map(f => `${f.emoji}${f.name}: ${f.stake}`).join(' | ')}`,
        'info'
      );

      if (winner.isYou) {
        setValidator('YOU'); setHasWonLottery(true);
        setLotteryWinner({ name: 'YOU', stake: yourNumber, isYou: true, emoji: '🧑‍💻' });
        setLotteryModalWinner({ name: 'YOU', emoji: '🧑‍💻', stake: yourNumber, isYou: true });
        speak('You won the validator lottery!');
        if (addLog) addLog(`🎰 BLOCK #${index}: YOU won with number ${yourNumber}`, 'success');
        setTimeout(() => { setIsSelecting(false); setLotteryModalWinner(null); }, 2500);
      } else {
        const friend = friendNumbers.find(f => f.name === winner.name);
        if (addLog) addLog(`🎲 LOTTERY: ${winner.emoji} ${winner.name} highest with ${winner.stake}`, 'info');
        setValidator(winner.name); setStake(winner.stake);
        setLotteryWinner({ ...friend, stake: winner.stake, isYou: false });
        setLotteryModalWinner({ name: winner.name, emoji: winner.emoji, stake: winner.stake, isYou: false });
        speak(`${winner.name} won the validator lottery!`);
        setTimeout(() => {
          setIsSelecting(false); setLotteryModalWinner(null);
          const sig    = sha256(hash + friend.privateKey).toString();
          const reward = calcReward(winner.stake);
          ...
          onUpdate(index, { hash, nonce, sender, receiver, amount, signature: sig, txSignature, signerPubKey, originalTxHash: savedOriginalTxHash, validator: winner.name, stake: winner.stake, createdWith: savedCreatedWith, friendSigned: true, validatorPubKey: friend.publicKey });
          playSuccess();
          speak(`${winner.name} signed the block.`);
          if (addLog) addLog(`🎰 BLOCK #${index}: ${winner.emoji} ${winner.name} signed — reward: ${reward} ETH`, 'info');
        }, 2500);
      }
    }, 2000);
  };
  ...
  // ── MINING RACE MODAL (rendered via portal to document.body) ──
  const miningRaceModal = showMiningRace ? ReactDOM.createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 999999, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(3px)' }}>
      <div style={{ background: '#020b02', border: '2px solid #00ff00', boxShadow: '0 0 60px #00ff0044, 0 0 120px #00ff0011', borderRadius: '12px', padding: '28px 32px', width: '60vw', maxWidth: '900px', minWidth: '600px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', fontFamily: 'monospace', animation: 'fadeInScale 0.2s ease', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', maxHeight: '280px', overflowY: 'auto', flexShrink: 0 }}>
  ...
  // ── LOTTERY MODAL (rendered via portal to document.body) ──
  const lotteryModal = isSelecting ? ReactDOM.createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 999999, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
      <div style={{ background: '#0a000f', border: '2px solid #d800ff', boxShadow: '0 0 60px #d800ff44, 0 0 120px #d800ff11', borderRadius: '16px', padding: '32px 40px', width: '55vw', maxWidth: '700px', minWidth: '480px', maxHeight: '90vh', overflowY: 'auto', fontFamily: 'monospace', animation: 'fadeInScale 0.3s ease', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginBottom: '16px' }}>
          {[{ name: 'YOU', emoji: '🧑‍💻', stakeVal: stake }, ...friendStakes.map(f => ({ ...f, stakeVal: f.stake }))].map((v, i) => {
            const maxStake = Math.max(stake, ...friendStakes.map(f => f.stake), 1);
  ...
  return (
    <>
      ...
      {/* Portals render directly to document.body — above everything */}
      {miningRaceModal}
      {lotteryModal}
